...
import { 
  ArrowLeft, Send, Image, Smile, Paperclip, Info, MoreVertical, History, 
  Palette, Plus, Moon, Sun, Trash, X, ChevronRight, Bot as BotIcon,
  Mic, MicOff
} from 'lucide-react';
...
function CharacterChat() {
  ...
  const [autoModeTimer, setAutoModeTimer] = useState<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    if (isAutoMode && messages.length > 0) {
      ...
      if (lastMessage.sender === 'ai') {
        ...
        words.forEach(word => {
          if (word.length > 4 && !['about', 'would', 'could', 'should', 'their'].includes(word)) {
        ...
        const positiveWords = ['happy', 'great', 'good', 'wonderful', 'excellent', 'amazing'];
        const negativeWords = ['sad', 'bad', 'terrible', 'unfortunate', 'sorry', 'worried'];
        ...
        const generateAutoMessage = () => {
          ...
          const responsePool = templates[context.lastResponseType as keyof typeof templates] || templates.statement;
        ...
        const timer = setTimeout(async () => {
          ...
          setMessage('');
          setMessages(prev => [...prev, { text: autoMessage, sender: 'user' }]);
          setIsLoading(true);
  ...
  const generateChatSuggestions = () => {
    ...
    if (messages.length === 0) {
      return character?.tags?.slice(0, 3).map(tag => `Tell me about ${tag}`) || [
    ...
    const keywords = messageText.split(/\s+/)
      .filter(word => word.length > 4)
      .filter(word => !['about', 'would', 'could', 'should', 'their', 'there', 'these', 'those'].includes(word));
    ...
    if (character?.personality?.interests && character.personality.interests.length > 0) {
      const randomInterest = character.personality.interests[Math.floor(Math.random() * character.personality.interests.length)];
      suggestions.push(`What do you think about ${randomInterest}?`);
    }
    
    // Add emotion-based suggestions
    if (messageText.includes('happy') || messageText.includes('glad') || messageText.includes('great')) {
      suggestions.push("I'm happy to hear that!");
    } else if (messageText.includes('sad') || messageText.includes('sorry') || messageText.includes('unfortunate')) {
  ...
  return (
    <div className="min-h-screen flex" style={{
      ...
      <aside className={`fixed left-0 top-0 bottom-0 w-80 backdrop-blur-sm transform transition-transform duration-300 ${
        ...
        <div 
          className="absolute inset-0 z-0"
          style={{
            background: `linear-gradient(to bottom, ${themes[activeTheme]?.colors.surface || '#27272a'}cc, ${themes[activeTheme]?.colors.background || '#18181b'}ee)`,
            borderRight: `1px solid ${themes[activeTheme]?.colors.border || '#27272a'}`
          }}
        />
        <div className="p-6 space-y-6 h-full overflow-y-auto relative z-10">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {character.tags?.map((tag) => (
                <span key={tag} className="px-3 py-1 bg-gold/20 text-gold rounded-full text-sm">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div>
            <h3 className="text-lg font-semibold text-gold mb-3">Personality Traits</h3>
            <div className="flex flex-wrap gap-2">
              {character.personality.traits.map((trait) => (
                <span key={trait} className="px-3 py-1 bg-zinc-700/50 rounded-full text-sm text-zinc-300">
                  {trait}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {character.personality.interests.map((interest) => (
                <span key={interest} className="px-3 py-1 bg-zinc-700/50 rounded-full text-sm text-zinc-300">
                  {interest}
                </span>
              ))}
            </div>
          ...
          <div>
            <h3 className="text-lg font-semibold text-gold mb-3">Unique Quirks</h3>
          ...
          <div>
            <h3 className="text-lg font-semibold text-gold mb-3">Communication Style</h3>
            <div className="space-y-3">
              <div>
                <p className="text-zinc-400 text-sm">Emotional Style</p>
                <p className="text-zinc-300">{character.personality.emotionalStyle}</p>
              </div>
              <div>
                <p className="text-zinc-400 text-sm">Speaking Style</p>
                <p className="text-zinc-300">{character.personality.speakingStyle}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Main Chat Area */}
      <main className={`flex-1 flex flex-col transition-all duration-300 ${showInfo ? 'ml-80' : 'ml-0'}`}>
        {/* Header */}
        <header className="fixed top-0 right-0 backdrop-blur-sm z-50 transition-all duration-300" style={{ 
          left: showInfo ? '320px' : '0',
          background: `${themes[activeTheme]?.colors.background || '#18181b'}95`,
          borderBottom: `1px solid ${themes[activeTheme]?.colors.border || '#27272a'}`
        }}>
          <div className="px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className={`icon-button ${showInfo ? 'text-gold' : 'text-zinc-400'}`}
              >
        ...
        {showThemes && (
          <div className="fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center" style={{
            background: 'rgba(0, 0, 0, 0.7)'
          }}>
            <div className="rounded-xl w-[800px] max-h-[90vh] flex flex-col animate-fade-in" style={{
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar">
                {/* Theme Categories */}
                {(['anime', 'movie', 'nature', 'emotion'] as const).map(category => (
        ...
        <div className="flex-1 overflow-y-auto pt-20 pb-32">
          <div className="max-w-3xl mx-auto px-6 space-y-6">
            ...
            >
              <p>{character.languages?.greeting || 'Greetings!'} I am {character.name}. {character.personality.background}</p>
              <div 
            ...
            {messages.map((msg, index) => (
              <div
                key={`msg-${index}`}
                className={`message-bubble ${msg.sender} animate-fade-in text-base relative ${
              ...
              >
                ...
                <div 
                  className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 rotate-45 ${
                    msg.sender === 'user' ? 'right-0 translate-x-2' : 'left-0 -translate-x-2'
                  }`}
                  style={{
                    background: msg.sender === 'user' 
                      ? themes[activeTheme]?.colors.primary || '#6366F1'
        ...
        {/* Chat Input */}
        <div className="fixed bottom-0 right-0 backdrop-blur-lg transition-all duration-300" style={{ 
          left: showInfo ? '320px' : '0',
          background: `${themes[activeTheme]?.colors.background || '#18181b'}95`,
          borderTop: `1px solid ${themes[activeTheme]?.colors.border || '#27272a'}`
        }}>
              ...
              <div className="relative flex-1">
                <textarea
                  ...
                  style={{
                    ...
                    border: `1px solid ${themes[activeTheme]?.colors.border || '#3f3f46'}`
                  }}
                  rows={1}
                  onKeyDown={(e) => {
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center space-x-2">
                  ...
                  <button 
                    ...
                    className={`p-2 rounded-full ${message.trim() && !isLoading ? 'text-white bg-nexus-blue-500 hover:bg-nexus-blue-600' : 'text-gray-400 bg-white/10'}`}
                  >

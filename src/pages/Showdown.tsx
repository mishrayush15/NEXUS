...
import { 
  ArrowLeft, Send, Bot, Home, Star, Settings, Plus, UserPlus, X, 
  Users, MessageSquare, Trash2, Info, Search, Crown, Check,
  Sparkles, Zap, RefreshCw, Image, Smile
} from 'lucide-react';
...
function Showdown() {
  ...
  const [recentlyAddedCharacter, setRecentlyAddedCharacter] = useState<string | null>(null);
      ...
      setTimeout(() => {
        ...
        if (char) {
          ...
          if (input.toLowerCase().includes('hello') || input.toLowerCase().includes('hi')) {
            responseText = `Hello there! ${char.languages?.greeting || 'Nice to meet you in this showdown!'}`;
          } else if (input.toLowerCase().includes('how are you')) {
            responseText = `I'm doing well, thanks for asking! As ${char.name}, ${personality.interests ? "I've been thinking about " + personality.interests[0] : "I'm always here to help."} How about you?`;
          } else if (input.toLowerCase().includes('?')) {
            responseText = `That's an interesting question! From my perspective as ${char.name}, I'd say ${personality.speakingStyle === 'Technical' ? 'it depends on several factors.' : 'there are many ways to look at it.'}`;
          } else {
            // Generic response using character traits
            responseText = `I hear what you're saying. As ${char.name}, ${personality.traits ? 'I tend to be ' + personality.traits[0].toLowerCase() + ', so ' : ''}I think that's ${Math.random() > 0.5 ? 'a fascinating point' : 'an interesting perspective'}.`;
          }
        } else {
        ...
        if (participant.id === activeParticipants[activeParticipants.length - 1].id) {
          setIsLoading(false);
        }
      }, delay);
      ...
      <div className="min-h-screen bg-gradient-to-br from-zinc-900 to-zinc-950 flex flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div 
            ...
            className={`flex-1 overflow-y-auto p-4 custom-scrollbar ${participants.length === 0 ? 'flex items-center justify-center' : ''}`}
            style={{
          ...
          >
            {participants.length === 0 ? (
              <div className={`max-w-md text-center p-8 bg-zinc-800/90 backdrop-blur-md rounded-2xl shadow-xl border border-zinc-700 ${animateIntro ? 'animate-fadeIn' : ''}`}>
                <div className="relative w-20 h-20 mx-auto mb-6">
                  <div className="absolute inset-0 bg-amber-500/20 rounded-full animate-pulse-slow"></div>
                  <Sparkles className="w-16 h-16 text-amber-500 relative z-10 animate-pulse-slow" style={{ animationDelay: "0.5s" }} />
                </div>
                <h2 className="text-2xl font-bold text-white mb-4">Start Your AI Showdown</h2>
                <p className="text-zinc-300 mb-8">
                  Create the ultimate conversation by bringing together up to 10 different AI characters.
                  Watch them interact with you and each other!
                </p>
                <button
                  onClick={toggleCharacterSelector}
              ...
              <div className="max-w-4xl mx-auto space-y-6 pt-4 pb-6">
                ...
                {messages.map((message, index) => (
                  <div 
                    key={message.id}
                    className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} relative message-appear`}
                  >
                    ...
                    <div className={`max-w-[80%] flex ${message.sender === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start gap-3`}>
                      <div className="relative">
                        <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 border-2 shadow-md" 
                          style={{borderColor: getSenderColor(message.sender)}}>
                          <img 
                        ...
                        {message.sender !== 'user' && (
                          <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-zinc-800 rounded-full flex items-center justify-center border border-zinc-700">
                            <Sparkles className="w-3 h-3" style={{color: getSenderColor(message.sender)}} />
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        {message.sender !== 'user' && (
                          <div className="font-medium mb-1 flex items-center space-x-1">
                            <span 
                              ...
                              style={{
                                background: `linear-gradient(to right, white, ${getSenderColor(message.sender)})`,
                            ...
                            <span className="text-xs text-zinc-500">{message.timestamp.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}</span>
                          </div>
                        )}
                        ...
                        <div 
                          ...
                          style={message.sender !== 'user' ? {
                            borderColor: `${getSenderColor(message.sender)}50`,
                            boxShadow: `0 4px 15px ${getSenderColor(message.sender)}10`
                          } : {}}
                        >
                          ...
                          {message.sender === 'user' && (
                            <div className="text-right">
                              <span className="text-xs text-zinc-800">{message.timestamp.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="max-w-[80%] flex flex-row items-start gap-3">
                      ...
                      <div className="bg-zinc-800/90 backdrop-blur-sm rounded-2xl px-4 py-3 border border-zinc-700 rounded-tl-none">
                        <div className="flex space-x-2">
                          <div className="animate-typing-dot" style={{ animationDelay: '0ms' }}></div>
                          <div className="animate-typing-dot" style={{ animationDelay: '500ms' }}></div>
                          <div className="animate-typing-dot" style={{ animationDelay: '1000ms' }}></div>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {recentlyAddedCharacter && (
            <div className="fixed top-24 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-zinc-900 to-zinc-800 border-l-4 border-amber-500 rounded-lg px-4 py-3 text-white shadow-2xl z-50 flex items-center space-x-2 animate-slideDown">
              ...
              <span><span className="text-amber-500 font-bold">{recentlyAddedCharacter}</span> has joined the showdown!</span>
            </div>
          )}

          {/* Participants Sidebar */}
          <div className={`transition-all duration-300 border-l border-zinc-800 bg-zinc-900/80 backdrop-blur-md overflow-y-auto flex flex-col ${sidebarExpanded ? 'w-72' : 'w-20'}`}>
            <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
              <h3 className={`font-bold text-white flex items-center ${sidebarExpanded ? 'text-lg' : 'sr-only'}`}>
              ...
              >
                {sidebarExpanded ? <X className="w-5 h-5" /> : <Users className="w-5 h-5" />}
              </button>
            </div>
            
            <div className={`flex-1 overflow-y-auto custom-scrollbar ${sidebarExpanded ? 'p-4' : 'p-2'} space-y-3`}>
              {participants.length === 0 ? (
                <div className={`text-center text-zinc-400 text-sm ${!sidebarExpanded && 'sr-only'}`}>
                  No characters added yet
                </div>
              ) : (
                <>
                  <div className={`bg-zinc-800/90 rounded-xl overflow-hidden shadow-inner ${sidebarExpanded ? 'p-3' : 'p-2'} border border-zinc-700 transition-all card-hover-effect`}>
                    <div className={`flex items-center ${sidebarExpanded ? 'space-x-3' : 'justify-center'}`}>
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-yellow-500 to-amber-600 flex items-center justify-center shadow-lg relative overflow-hidden">
                        <span className="text-zinc-900 font-bold relative z-10">U</span>
                      ...
                      {sidebarExpanded && (
                        <div>
                          ...
                          <div className="text-xs text-zinc-500">Human User</div>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {participants.map((participant) => (
                    <div 
                      key={participant.id}
                      className={`${participant.isActive ? 'bg-zinc-800/90' : 'bg-zinc-900/70'} rounded-xl overflow-hidden shadow-md transition-all duration-200 hover:shadow-lg group relative ${sidebarExpanded ? 'p-3' : 'p-2'} border border-zinc-700 card-hover-effect`}
                      style={participant.isActive ? {
                    ...
                    >
                      <div className={`flex items-center ${sidebarExpanded ? 'space-x-3' : 'justify-center'}`}>
                        <div className="relative flex-shrink-0">
                          <div 
                            className={`w-10 h-10 rounded-full overflow-hidden border-2 shadow-lg ${!participant.isActive && 'grayscale opacity-70'}`}
                            style={{borderColor: participant.color || '#4F9EFA'}}
                          >
                            <img 
                        ...
                        {sidebarExpanded && (
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-zinc-500 truncate">{participant.role}</div>
                          </div>
                        )}
                      </div>
                      ...
                      {sidebarExpanded && (
                        <div className="absolute top-3 right-3 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button 
                            onClick={() => toggleCharacterActive(participant.id)}
                            className={`p-1.5 rounded-full transition-colors ${participant.isActive ? 'bg-zinc-700 text-white hover:bg-zinc-600' : 'bg-zinc-800 text-zinc-500 hover:bg-zinc-700 hover:text-white'}`}
                          >
                            <MessageSquare className="w-3.5 h-3.5" />
                          </button>
                          <button 
                            onClick={() => removeCharacterFromChat(participant.id)}
          ...
          <div className="max-w-3xl mx-auto">
            <div className="relative w-full rounded-2xl bg-zinc-800/90 border border-zinc-700 overflow-hidden shadow-xl transition-all hover:border-zinc-600">
              <textarea
                ...
                placeholder={participants.length === 0 ? "Add characters to start chatting..." : "Type a message... (Alt + A for auto-reply)"}
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center space-x-2">
                ...
                >
                  <span className="text-xs font-medium bg-zinc-700/50 px-2 py-1 rounded">Auto (A)</span>
                </button>
                <button
                  onClick={handleSendMessage}
                  disabled={!input.trim() || isLoading || participants.length === 0}
            ...
            <div className="relative w-full max-w-5xl max-h-[85vh] bg-gradient-to-br from-zinc-900 to-zinc-950 rounded-2xl overflow-hidden border border-zinc-700 shadow-2xl animate-fadeIn flex flex-col">
              <div className="p-6 border-b border-zinc-800 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-xl font-bold text-white">Select AI Characters</h2>
                </div>
                <button
                  onClick={toggleCharacterSelector}
                ...
                {Object.keys(characters).length === 0 ? (
                  ...
                  <div className="flex items-center justify-center h-64">
                    <div className="text-center">
                      ...
                      <p className="text-zinc-400">No characters found matching your search</p>
                      <button
                        onClick={() => setCharacterSearch('')}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Object.keys(characters)
                      ...
                      .map(id => {
                        const char = characters[id];
                        const isSelected = participants.some(p => p.id === id);
                          ...
                          <div 
                            ...
                            className={`
                              ...
                              ${hasReachedLimit && !isSelected 
                                ...
                                : 'cursor-pointer'}
                            `}
                            style={isSelected ? {'--glow-color': 'rgba(245, 158, 11, 0.15)'} as React.CSSProperties : {}}
                            onClick={() => !hasReachedLimit || isSelected ? addCharacterToChat(id) : null}
                          >
                            <div className="flex p-4 bg-zinc-800/90 relative">
                              <div className="mr-4">
                                <div 
                                  className={`w-16 h-16 rounded-xl overflow-hidden border-2 shadow-lg ${isSelected ? 'border-amber-500' : 'border-zinc-600'}`}
                                >
                                  <img 
                                    src={char.image} 
                                    alt={char.name}
                              ...
                              <div className="flex-1">
                                <div className="flex items-start justify-between">
                                  <div>
                                    <h3 className={`font-bold text-lg ${isSelected ? 'text-amber-500' : 'text-white'}`}>
                                      {char.name}
                                    </h3>
                                    <p className="text-zinc-400 text-sm">{char.role}</p>
                                  </div>
                            ...
                            <div className="p-3 text-sm border-t border-zinc-700 bg-zinc-900/70 text-zinc-400 flex justify-between items-center">
                              <span className="flex items-center">
                                ...
                                {(char.personality?.traits || []).slice(0, 1).join(', ')}
                              </span>
                              <button 
                                className={`
                              ...
                              >
                                {isSelected ? 'Selected' : (hasReachedLimit ? 'Limit Reached' : 'Add')}
                              </button>
                            </div>
              ...
              <div className="p-4 border-t border-zinc-800 flex items-center justify-between bg-zinc-900/80">
                <div className="text-zinc-400">
                  {participants.length > 0 ? (
                    <span>Selected: <span className="text-amber-500 font-bold">{participants.length}/10</span> characters</span>
                  ) : (

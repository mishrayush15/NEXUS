...
// Define interface for announcement objects
...
const menuItems = [
  ...
  { icon: Users, label: 'Showdown', active: false },
  { icon: Plus, label: 'Create Buddy', active: false },
  { icon: Users, label: 'Companion', active: false },
  { icon: BookOpen, label: 'Genres', active: false },
...
const tagCategories = [
  ...
  {
    name: "Interests",
    tags: ["Education", "Entertainment", "Writing", "Creativity", "Science", "Philosophy"]
  },
  {
    name: "Fields",
...
// Update announcement data to showcase hot characters and key features
const announcements: Announcement[] = [
  {
    id: 1,
    title: "Hinata",
    description: "Beautiful, strong, and determined. Hinata is a character that is not afraid to stand up for what she believes in.",
    cta: "Chat Now",
    ctaLink: "/chat/hinata",
  ...
  },
  {
    id: 2,
    title: "AI Showdown",
    description: "Create epic conversations with multiple AI characters. Watch them interact with each other in real-time.",
    cta: "Start Showdown",
    ctaLink: "/showdown",
    color: "from-amber-900/80 to-orange-900/70",
    image: "https://i.pinimg.com/736x/8b/b7/5c/8bb75cb0245e6abdd5559da02ed29332.jpg",
    tagline: "FEATURED EXPERIENCE"
  },
  {
    id: 3,
    title: "Create Your Own AI Buddy",
    description: "Design a custom AI companion with unique personality traits, interests, and appearance.",
    cta: "Create Now",
    ctaLink: "/create-buddy",
    color: "from-emerald-900/80 to-teal-900/70",
    image: "https://i.pinimg.com/736x/30/e5/ec/30e5ec8f1ec47882de36ee53e26fada9.jpg",
...
const genreCategories = [
  { 
    ...
    slugs: [
      ...
      'mai-sakurajima', 'winry-rockbell', 'tsunade-senju', 'rukia-kuchiki', 'albedo',
      'historia-reiss', 'annie-leonhart', 'farnese-berserk', 'android-18', 'fern',
      'darkness', 'aqua', 'megumin', 'yor-forger', 'makima', 'power', 'saber',
      'rin-tohsaka', 'violet-evergarden', 'ryuko-matoi', 'toga-himiko', 'ochaco-uraraka',
      'momo-yaoyorozu', 'lucy-heartfilia', 'erza-scarlet', 'nezuko', 'bulma',
      'hestia', 'roxy-migurdia', 'yuno-gasai', 'revy', 'nami', 'nico-robin'
    ],
  ...
  { 
    ...
    slugs: [
      ...
      'zoro', 'sanji', 'guts', 'griffith', 'light-yagami', 'lelouch', 'hisoka',
      'alucard', 'kirito', 'itachi', 'vegeta', 'minato', 'jiraiya', 'kira-yamato',
      'archer-emiya', 'gilgamesh', 'edward-elric', 'roy-mustang', 'kaneki-ken',
      'tanjiro', 'zenitsu', 'inosuke', 'subaru', 'kazuma', 'jean-kirstein',
      'l-lawliet', 'yagami-light', 'spike-spiegel', 'vash', 'mugen', 'usopp'
    ],
  ...
  { 
    ...
    slugs: [
      ...
      'vampire-prince', 'alpha-werewolf', 'college-quarterback', 'mysterious-neighbor',
      'bodyguard', 'rockstar', 'college-professor', 'surgeon', 'best-friends-brother',
      'celebrity-crush', 'arranged-marriage', 'childhood-enemy'
    ],
  ...
  { 
    ...
    slugs: [
      ...
      'learning-companion', 'history-professor', 'philosophy-mentor',
      'personal-assistant', 'business-consultant', 'marketing-strategist', 'design-mentor',
      'project-manager', 'data-scientist', 'startup-advisor', 'job-interview-coach',
      'resume-writer', 'time-management-coach', 'public-speaking-coach', 'debate-coach',
      'language-learning-partner', 'book-summarizer', 'technical-writer', 'quantum-physics-teacher',
      'statistics-tutor', 'chemistry-lab-assistant', 'astronomy-professor', 'ai-ethics-expert'
    ],
...
function AiChat() {
  ...
  useCharacterViews(setViews, useMemo(() => {
  ...
  const handleCreateCharacter = (character: any) => {
    ...
    const newCharacter = {
      ...
      image: character.image || 'https://images.unsplash.com/photo-1675789652575-0a5d2425b6c2?auto=format&fit=crop&w=300&h=300',
  ...
  const categoryCharacters = useMemo(() => {
    ...
    const crowdPleasersCharacters = allCharacters
      .filter(([slug, _]) => !trendingSlugs.includes(slug))
      .map(([slug, character]) => ({ slug, character, views: views[slug] || 0 }))
      .sort((a, b) => b.views - a.views)
      .slice(0, 10)
      .map(({ slug, character }) => [slug, character] as [string, any]);
      ...
      <main className="ml-56 px-6 pt-24 pb-12">
        <div className="container mx-auto">
          ...
          {showGenres && (
            <div>
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-white mb-2">Character Genres</h2>
                <p className="text-zinc-400">Explore different categories of AI companions</p>
              </div>
                    ...
                    genre.id === activeGenre && (
                      <div key={genre.id} className="animate-fadeIn">
                        <div className="flex items-center mb-6">
                          <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${genre.bgColor} flex items-center justify-center mr-4`}>
                            {genre.icon}
                          </div>
                          <div>
                            <h3 className="text-2xl font-bold text-white">{genre.name}</h3>
                            <p className="text-zinc-400">{genre.description}</p>
                          </div>
                        </div>
                    ...
                    >
                      ...
                      <div className="flex -space-x-2 mb-4">
                        ...
                        {genre.slugs.length > 5 && (
                          <div className="w-8 h-8 rounded-full border-2 border-white bg-black/70 flex items-center justify-center text-xs text-white font-bold" style={{ zIndex: 0 }}>
                            +{genre.slugs.length - 5}
                          </div>
                        )}
                      </div>
            ...
            <>
              ...
              <div className="mb-12">
                <div className="flex items-center mb-5">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-white">Hot Characters</h2>
                    <div className="h-1 w-12 mt-1 bg-gradient-to-r from-red-500 to-amber-500 rounded-full"></div>
                    <p className="text-zinc-400 text-sm mt-1">Characters everyone's talking about</p>
                  </div>
                </div>
                  ...
                  {Object.entries(characters)
                    ...
                    .filter(([slug]) => {
                      ...
                      const categorySlugs = new Set([
                        ...
                        ...(categoryCharacters.boredomBuster || []).map(c => c.slug),
          ...
          {showFavorites && (
            <div>
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-white mb-2">My Starred Characters</h2>
                <p className="text-zinc-400">All your favorite characters in one place</p>
              </div>
              ...
              {favorites.length === 0 ? (
                <div className="bg-zinc-800/50 rounded-xl p-8 text-center">
                  ...
                  <p className="text-zinc-400 mb-6">Click the star icon on any character to add them to your favorites</p>
                  <button
